...
项目中的桌面资源被同步到用户配置目录，并备份一份受管理的安装后基线到：

```text
/usr/local/share/cachyos-desktop/
```

Niri 主配置会按目标用户的 `$HOME` 渲染并完整覆盖。它会加载：
...
`cachyos-extras.kdl` 还会加载 `dms/wpblur.kdl`。渲染后的主配置也会写入
`/usr/local/share/cachyos-desktop/assets/niri/config.kdl`，供 `cachyos-config`
进行准确的一致性检查和恢复。
...
- 安装 DMS 专用 PAM 配置；
- 将 `assets/dms/desktop-settings.json` 合并到 DMS 设置；
- 固定当前使用的动态主题方案、圆角、光标、模糊和 Overview Overlay 设置；
...
- DMS Overview Overlay 启用，并使用完整启动器样式。
...
动态颜色由所选壁纸计算。项目不复制当前用户的个人壁纸文件，因此新机需自行选择壁纸，
配色结果也可能与本机不同。
...
安装器直接部署稳定的 Niri 主配置、快捷键和项目扩展：

```text
assets/niri/config.kdl
  -> ~/.config/niri/config.kdl
assets/niri/binds.kdl
  -> ~/.config/niri/dms/binds.kdl
assets/niri/cachyos-extras.kdl
  -> ~/.config/niri/cachyos-extras.kdl
```

`colors.kdl`、`layout.kdl`、`alttab.kdl`、`outputs.kdl`、`cursor.kdl` 和
`wpblur.kdl` 由 DMS 根据 `assets/dms/desktop-settings.json` 自动生成，项目不会复制或
强制覆盖这些生成文件。这会复刻稳定的 Niri 行为、窗口规则、Overview 设置和整套快捷键；
个人壁纸、账号和应用私有状态不在复制范围内，动态配色也取决于新机所选壁纸。
...
```bash
scp -r /home/skynit/workspace/CachyOS-Niri-Installer \
